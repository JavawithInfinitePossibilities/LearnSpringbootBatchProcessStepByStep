...
@SpringBootTest(classes = Division08WorkingWithitemWriterJDBC.class)
@SpringBatchTest
@Import(MyTestConfigClass.class)
class Division08WorkingWithitemWriterJDBCTest {
	...
	void testStep() throws Exception {
		JobParameters jobParameters = new JobParametersBuilder()
				.addString("inputFiles", "CustomerDetailsMultipleSchema-14").addString("outputFile", "Customer-14.jdbc")
				.toJobParameters();
		JobExecution launchJob = jobLauncherTestUtils.launchStep("eighth-Step", jobParameters);
		launchJob.getStepExecutions().forEach(item -> System.out.println("Step name is : " + item.getStepName()));
		assertEquals("eighth-Step", ((List<StepExecution>) launchJob.getStepExecutions()).get(0).getStepName());
		assertEquals(ExitStatus.COMPLETED, launchJob.getExitStatus());
	}
}
