...
public class CustomItemReaderForFlatFile {
	...
	/**
	 * This will make the Item Reader full into an infinite loop. This is for only
	 * for testing.
	 */
	@Bean
	@StepScope
	public JsonItemReader<Car> jsonItemReader(@Value("#{jobParameters['inputFiles']}") ClassPathResource resource) {
		/*JsonItemReader<Car> jsonItemReader = new JsonItemReader<Car>() {
			@Override
			protected Car doRead() throws Exception {
				Car car = super.doRead();
				if (car.getId().equals(100)) {
					throw new NullPointerException(car.toString());
				}
				return super.doRead();
			}
		};*/
