...
@SpringBootTest(classes = Division08WorkingWithitemWriterJSON.class)
@SpringBatchTest
@Import(MyTestConfigClass.class)
class Division08WorkingWithitemWriterJSONTest {
	...
	void testStep() throws Exception {
		JobParameters jobParameters = new JobParametersBuilder()
				.addString("inputFiles", "CustomerDetailsMultipleSchema-12").addString("outputFile", "Customer-12.json")
				.toJobParameters();
		JobExecution launchJob = jobLauncherTestUtils.launchStep("seventh-Step", jobParameters);
		launchJob.getStepExecutions().forEach(item -> System.out.println("Step name is : " + item.getStepName()));
		assertEquals("seventh-Step", ((List<StepExecution>) launchJob.getStepExecutions()).get(0).getStepName());
		assertEquals(ExitStatus.COMPLETED, launchJob.getExitStatus());
	}
