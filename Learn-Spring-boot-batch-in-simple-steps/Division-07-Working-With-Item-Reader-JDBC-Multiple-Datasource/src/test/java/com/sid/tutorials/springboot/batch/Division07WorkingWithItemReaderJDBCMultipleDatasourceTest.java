...
@SpringBootTest(classes = Division07WorkingWithItemReaderJDBCMultipleDatasource.class)
@SpringBatchTest
@Import(MyTestConfigClass.class)
class Division07WorkingWithItemReaderJDBCMultipleDatasourceTest {
	...
	void testStep() throws Exception {
		JobParameters jobParameters = new JobParametersBuilder().addString("inputFiles", "CustomerDetailsMultipleSchema")
				.toJobParameters();
		JobExecution launchJob = jobLauncherTestUtils.launchStep("fifth-Step", jobParameters);
		launchJob.getStepExecutions().forEach(item -> System.out.println("Step name is : " + item.getStepName()));
		assertEquals("fifth-Step", ((List<StepExecution>) launchJob.getStepExecutions()).get(0).getStepName());
		assertEquals(ExitStatus.COMPLETED, launchJob.getExitStatus());
	}
