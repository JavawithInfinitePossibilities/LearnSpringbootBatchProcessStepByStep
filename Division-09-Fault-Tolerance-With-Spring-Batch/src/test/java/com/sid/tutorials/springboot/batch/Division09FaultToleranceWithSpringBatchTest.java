...
@SpringBootTest(classes = Division09FaultToleranceWithSpringBatch.class)
@SpringBatchTest
@Import(MyTestConfigClass.class)
class Division09FaultToleranceWithSpringBatchTest {
	...
	void testStep() throws Exception {
		JobParameters jobParameters = new JobParametersBuilder().addString("inputFiles", "cars.json")
				.addString("outputFile", "car.json").toJobParameters();
		JobExecution launchJob = jobLauncherTestUtils.launchStep("First-Step", jobParameters);
		launchJob.getStepExecutions().forEach(item -> System.out.println("Step name is : " + item.getStepName()));
		assertEquals("First-Step", ((List<StepExecution>) launchJob.getStepExecutions()).get(0).getStepName());
		assertEquals(ExitStatus.COMPLETED, launchJob.getExitStatus());
	}
