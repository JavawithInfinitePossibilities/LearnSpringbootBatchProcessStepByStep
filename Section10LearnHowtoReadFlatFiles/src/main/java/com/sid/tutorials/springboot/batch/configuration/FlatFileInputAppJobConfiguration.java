...
public class FlatFileInputAppJobConfiguration {
	...
	public FlatFileItemReader<Customer> customerCustomItemReader() {
		...
		defaultLineMapper.setFieldSetMapper((FieldSet fieldSet) -> {
			return new Customer(fieldSet.readLong("id"), fieldSet.readString("firstName"), 
					fieldSet.readString("lastName"), fieldSet.readDate("birthdate", "dd-MM-yyyy HH:mm"));
		});
	...
	@Bean
	public Step step1() {
		return stepBuilderFactory.get("stepReaderCustomerFlatFile1Start").<Customer, Customer>chunk(100).reader(
				customerCustomItemReader()).writer(customItemWriter()).build();
	}
