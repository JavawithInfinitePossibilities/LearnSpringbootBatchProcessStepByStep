...
@SpringBootTest(classes = Division08WorkingWithitemWriter.class)
@SpringBatchTest
@Import(MyTestConfigClass.class)
class Division08WorkingWithitemWriterTest {
	...
	void testStep() throws Exception {
		JobParameters jobParameters = new JobParametersBuilder()
				.addString("inputFiles", "CustomerDetailsMultipleSchema-11").addString("outputFile", "Customer-11.csv")
				.toJobParameters();
		JobExecution launchJob = jobLauncherTestUtils.launchStep("sixth-Step", jobParameters);
		launchJob.getStepExecutions().forEach(item -> System.out.println("Step name is : " + item.getStepName()));
		assertEquals("sixth-Step", ((List<StepExecution>) launchJob.getStepExecutions()).get(0).getStepName());
		assertEquals(ExitStatus.COMPLETED, launchJob.getExitStatus());
	}
